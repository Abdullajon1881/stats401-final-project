...
from build_web_data import WEB_DATA_DIR, WEB_FILES  # noqa: E402
...
def validate_files() -> bool:
    ...
    vendor = SITE_DIR / "vendor" / "d3.v7.min.js"
    if vendor.exists():
        head = vendor.read_text(encoding="utf-8", errors="ignore")[:200]
        check("d3js.org v7" in head,
              f"vendored D3 is v7 ({cfg.human_size(cfg.file_size_bytes(vendor))})")
    else:
        check(False, "D3 is vendored locally at site/vendor/d3.v7.min.js", critical=False)
    return ok
...
# ---------------------------------------------------------------------------
# 7. the page reads its numbers from JSON
# ---------------------------------------------------------------------------
# Any of these appearing literally in app.js or index.html would mean a headline
# figure was typed in rather than read from site/data at runtime.
FORBIDDEN_LITERALS = [
...
def validate_no_hardcoded_metrics() -> None:
    section("7. The page reads its figures from JSON, not from literals")
    for name in ("app.js", "index.html"):
        path = SITE_DIR / name
        text = path.read_text(encoding="utf-8")
        found = sorted({lit for lit in FORBIDDEN_LITERALS if lit in text})
        check(not found, f"site/{name} hard-codes no city or district metric "
                         f"(found: {found or 'none'})")

    app = (SITE_DIR / "app.js").read_text(encoding="utf-8")
    check("city_summary.json" in app, "app.js loads city_summary.json")
    check("districts.geojson" in app, "app.js loads districts.geojson")
    check("population_density.geojson" in app, "app.js loads population_density.geojson")
    check("metro_isochrone_10min.geojson" in app, "app.js loads the metro isochrone")

    html = (SITE_DIR / "index.html").read_text(encoding="utf-8")
    for key in ("metro_pct", "bus_only_pct", "underserved_pct", "analysis_population"):
        check(f'data-bind="{key}"' in html,
              f"index.html binds {key} from JSON rather than printing a literal")

    # the ranking must be derived, not typed in
    check(re.search(r"sort\s*\(", app) is not None,
          "app.js sorts the districts rather than relying on a fixed order")
    check("descending" in app and "metro_access_pct" in app,
          "the dot plot ranks on metro_access_pct read from the data")

    check("polygon intersection" in html,
          "the page carries the display-layer caveat about polygon intersection")
    check("aria-pressed" in html or "aria-pressed" in app,
          "layer controls expose pressed state to assistive technology")
    check("<button" in html, "layer controls are real buttons, not clickable divs")
