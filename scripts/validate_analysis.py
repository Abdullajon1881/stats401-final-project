...
def validate_districts(manifest: dict) -> pd.DataFrame | None:
    section("2. District access metrics")
    ...
    check(bool(finite), "no NaN or inf in numeric district metrics")
...
def validate_speed_sensitivity() -> None:
    section("4. Walking-speed sensitivity")
    ...
    print(f"    {'speed':>6s} {'metro%':>8s} {'bus-only%':>10s} "
          f"{'unserved%':>10s} {'combined%':>10s}")
    for _, r in city.iterrows():
        print(f"    {r.speed_kmh:6.1f} {r.metro_access_pct:8.2f} {r.bus_only_pct:10.2f} "
              f"{r.underserved_pct:10.2f} {r.combined_pct:10.2f}")
    ...
    for name, group in districts.groupby("district_name"):
        ...
        if np.any(np.diff(ordered.metro_access_pct.to_numpy()) < -PCT_TOL):
            bad.append(name)
    check(not bad, f"district metro access is monotonic in speed (violations: {bad or 'none'})")


def validate_surface_sensitivity() -> None:
    section("5. Population-surface sensitivity (2020 vs 2026)")
...
def validate_geometry() -> None:
    section("6. Boundary and service-area geometry")
    ...
    for path, label in ((cfg.METRO_ISOCHRONE_FILE, "metro"), (cfg.BUS_ISOCHRONE_FILE, "bus")):
...
def validate_access_points_and_snapping(manifest: dict) -> None:
    section("7. Access points and snapping")
    ...
    snap = pd.read_csv(cfg.TRANSIT_SNAP_DIAGNOSTICS_FILE)
    check(set(snap.label) == {"metro", "bus"}, "snap diagnostics cover metro and bus")
    for _, row in snap.iterrows():
        print(f"    {row.label:6s} n={int(row['count']):5d}  min={row.min_m:7.1f}  "
              f"median={row.median_m:7.1f}  p95={row.p95_m:8.1f}  "
              f"p99={row.p99_m:8.1f}  max={row.max_m:9.1f} m")
        check(bool(np.isfinite([row.min_m, row.median_m, row.p95_m, row.p99_m, row.max_m]).all()),
              f"{row.label} snap statistics finite")
        check(row.median_m < 100.0,
              f"{row.label} median snap offset is small ({row.median_m:.1f} m)")
        check(row.max_m < 2000.0,
              f"{row.label} worst snap offset is under 2 km ({row.max_m:.1f} m)",
              critical=False)

    cells = manifest.get("population_cells", {})
    diagnostics = cells.get("snap_diagnostics", {})
    if diagnostics:
        print(f"    cells  n={diagnostics.get('count'):,}  "
              f"min={diagnostics.get('min_m'):.1f}  median={diagnostics.get('median_m'):.1f}  "
              f"p95={diagnostics.get('p95_m'):.1f}  p99={diagnostics.get('p99_m'):.1f}  "
              f"max={diagnostics.get('max_m'):.1f} m")
        check(float(diagnostics.get("median_m", 1e9)) < 200.0,
              f"population-cell median snap offset is small "
              f"({diagnostics.get('median_m'):.1f} m)")
        check(float(diagnostics.get("max_m", 1e9)) < 5000.0,
              f"worst population-cell snap offset under 5 km "
              f"({diagnostics.get('max_m'):.1f} m)", critical=False)

    stranded_metro = float(cells.get("population_without_metro_source", 0.0))
    stranded_bus = float(cells.get("population_without_bus_source", 0.0))
    ...
    check(stranded_metro >= 0 and math.isfinite(stranded_metro),
          "disconnected metro population is a finite non-negative number")


def validate_manifest_counts(manifest: dict, table: pd.DataFrame | None) -> None:
    section("8. Analysis manifest")
...
def validate_cell_partition() -> None:
    """Re-derive the partition from the cached per-cell table when available."""
    section("9. Cell-level partition (from the cached per-cell table)")
    ...
    check(abs(recomputed - table.metro_access_population.sum()) <= POP_TOL,
          f"recomputed metro population matches the district table "
          f"({recomputed:,.2f})")
...
def main() -> int:
    ...
    table = validate_districts(manifest)
    validate_city_summary(table)
    validate_speed_sensitivity()
    validate_surface_sensitivity()
    validate_geometry()
    validate_access_points_and_snapping(manifest)
    validate_manifest_counts(manifest, table)
